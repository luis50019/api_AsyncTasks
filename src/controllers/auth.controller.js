  ...
  try {
    ...
    res
      .cookie("access_token", token, {
        ...
        secure: true, // 🔹 Solo permite cookies en HTTPS (Requerido en producción)
        sameSite: "None", // 🔹 Permite compartir cookies entre dominios diferentes
      })
      .json({
        id: userSaved._id,
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!userFound)
      return res.status(400).json({ message: "The email not exist" });

    const isMatch = await bcrypt.compare(password, userFound.password);
    if (!isMatch)
      return res.status(400).json({ message: "The password is not match" });
...
export const logout = async (req, res) => {
  res.clearCookie("access_token").status(200).json("The user is logout");
};
